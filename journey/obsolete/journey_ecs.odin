...
register_component :: proc(world : $W/^$World, $component_type : typeid, size : int, allocator : mem.Allocator = context.allocator) -> int 
...
index_component :: proc(world : $W/^$World, storage_index : int, index : int, $component_type : typeid) -> ^component_type
where intrinsics.type_is_struct(component_type){

    return internal_sparse_index_component(&world.component_stores.component_sparse[storage_index],index, component_type)
}
...
get_components_unsafe :: proc(world : $W/^$World, storage_index : int, len : int, $component_type :typeid) -> []component_type
where intrinsics.type_is_struct(component_type){
    return internal_sparse_fetch_components_upto(& world.component_stores.component_sparse[storage_index], component_type, len)
}
...
deinit_component_store :: proc(component_store : $C/^$ComponentStore){
    ...
    delete(component_store.query_cache)
}
...
@(private)
internal_sparse_index_entity :: #force_inline proc(component_sparse : $S/^$ComponentSparse, index : int) -> uint{
...
@(private)
internal_sparse_fetch_components_upto :: #force_inline proc(component_sparse : $S/^$ComponentSparse, $component_type : typeid, len : int) -> []component_type{
    return ([^]component_type)(component_sparse.component_blob)[:len]
}
